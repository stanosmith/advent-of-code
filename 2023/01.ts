...
const input = parseInput(`2xjzgsjzfhzhm1
...
const testInput = parseInput(`1abc2
pqr3stu8vwx
a1b2c3d4e5f
treb7uchet
`)
...
const answer = _.sum(
  _.map(testInput, (val) =>
    _.filter(val.split(''), (character) => parseInt(character)),
  )
    .map((numbers) => `${numbers[0]}${_.last(numbers)}`)
    .map((calibrationValue) => parseInt(calibrationValue)),
)
...
const testInputPartTwo = parseInput(`two1nine
eightwothree
abcone2threexyz
xtwone3four
4nineeightseven2
...
const answerPartTwo = _.sum(
  input
    ...
    .map(_.flatten)
    .map((line) => _.filter(line))
    .map((line) => line.map((val) => digits[val] || val))
    .map((numbers) => `${numbers[0]}${_.last(numbers)}`)
    .map((calibrationValue) => parseInt(calibrationValue)),
)
